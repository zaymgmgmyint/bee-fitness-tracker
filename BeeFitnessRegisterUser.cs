...
namespace BeeFitnessTracker
{
    public partial class BeeFitnessRegisterUser : Form
    {
        ...
        private void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            {
                // Pasword must be 6-15 characters long and contain at least one uppercase letter,
                // one lowercase letter, one digit, and one special character
                Regex regex = new Regex(@"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,15}$");
                if (!regex.IsMatch(txtPassword.Text.Trim()))
                {
                    MessageBox.Show("Password must be 6-15 characters long and contain at least one uppercase letter, one lowercase letter, one digit, and one special character");
                    txtPassword.Focus();
                    return;
                }
